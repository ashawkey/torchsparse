...
# avg voxelization.
# feat: [N, c], idx: [N], cnt: [N1]
# return [N1, c]
class VoxelizeGPU(Function):
    @staticmethod
    def forward(ctx, feat, idx, cnt):
        out = torchsparse_cuda.insertion_forward(feat.float().contiguous(),idx.int().contiguous(), cnt)
    ...
    def backward(ctx, top_grad):
        ...
        bottom_grad = torchsparse_cuda.insertion_backward(top_grad.float().contiguous(), idx, cnt, N)
