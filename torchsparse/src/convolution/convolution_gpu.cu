...
__global__ void inplace_convolution(const int n, const float *in_feat,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    // Pytorch tensors in C-ordering with in_nchannel x out_nchannel
    float tmp = 0.0;
...
params:
  A: [N, fin], wA = fin, hA = N
  B: [fin, fout], wB = fout, hB = fin
  C: [M, fout]
  C += A * B (loop for each kernel B)
 */
__global__ void matmul(const float *A, const int wA, const int hA,
  ...
  // TODO: atomicAdd(&C[wB * out_row + x], Csub); // For conv transpose, it might fail due to overlapping outputs
}
...
void ConvolutionBackwardKernelGPU(
  ...
  for (int k = 0; k < kernel_volume; k++) {
    ...
    dim3 threads(SHARED_BLOCK_SIZE, SHARED_BLOCK_SIZE);
    dim3 grid((in_nchannel + threads.x - 1) / threads.x, (n_active_in_volume + threads.y - 1) / threads.y);
...
void scatter_launch(const int n_in, const int n_out, const int c, 
                               const float *in_feat, float *out_feat, const int *kmap,
                               const bool transpose){
    scatter_kernel<<<ceil((double)(n_in * c) / 256), 256>>>(n_in, n_out, c, in_feat, out_feat, kmap, transpose);
}
...
void gather_launch(const int n_k, const int n_in, const int c, 
                               const float *in_feat, float *out_feat, const int *kmap,
                               const bool transpose){
    gather_kernel<<<ceil((double)(n_k * c) / 256), 256>>>(n_k, n_in, c, in_feat, out_feat, kmap, transpose);
}

///////////////////////////////////
/*
    # depth-wise convolution: features=[N, fin], kernel=[K, fin] -> output=[M, fin]

    output_features = torch.zeros((sizes[1], features.shape[1])).to(features.device) # [M, fin]

    nmap = idx_query[0].long() # [N, 2], from -> to
    offsets = idx_query[1].numpy() # [K], length of nmap for each kernel
    offset = 0
    for k in range(kernel.shape[0]):
        output_features[nmap[offset:offset+offsets[k], 1]] += features[nmap[offset:offset+offsets[k]:, 0]] * kernel[[k], :, 0]
        offset += offsets[k]
*/
...
__global__ void dwconv_forward(
  const float* __restrict__ d_in_feat,     // [N, fin]
  float* d_out_feat,                       // [M, fin]
  const float* __restrict__ d_kernel,      // [fin]
  const int* __restrict__ neighbor_map,    // [N', 2]
  const int nmap_size,        
  const int in_nchannel,
  const bool transpose
){

  int tid = blockIdx.x * blockDim.x + threadIdx.x; // [0, in_nchannel)
  int cid = blockIdx.y * blockDim.y + threadIdx.y; // [0, nmap_size)
  
  if (cid < in_nchannel && tid < nmap_size) {
    int f = neighbor_map[tid * 2];
    int t = neighbor_map[tid * 2 + 1];
    
    if (transpose) swap(&f, &t);

    //d_out_feat[t * in_nchannel + cid] += d_in_feat[f * in_nchannel + cid] * d_kernel[cid];
    atomicAdd(&d_out_feat[t * in_nchannel + cid], d_in_feat[f * in_nchannel + cid] * d_kernel[cid]);
  }
}

__global__ void dwconv_backward(
  const float* __restrict__ d_in_feat,     // [N, fin]
  const float* __restrict__ d_kernel,      // [fin]
  const int* __restrict__ neighbor_map,    // [N', 2]
  const int nmap_size,        
  const int in_nchannel,
  const float* __restrict__ d_grad_out_feat, // [M, fin]
  float* d_grad_in_feat,                     // [N, fin]
  float* d_grad_kernel,                       // [fin]
  const bool transpose
){
  ...
  if (cid < in_nchannel && tid < nmap_size) {
    ...
    //d_grad_in_feat[f * in_nchannel + cid] += d_kernel[cid] * d_grad_out_feat[t * in_nchannel + cid];
    atomicAdd(&d_grad_in_feat[f * in_nchannel + cid], d_kernel[cid] * d_grad_out_feat[t * in_nchannel + cid]);
    
    //d_grad_kernel[cid] += d_in_feat[f * in_nchannel + cid] * d_grad_out_feat[t * in_nchannel + cid];
    atomicAdd(&d_grad_kernel[cid], d_in_feat[f * in_nchannel + cid] * d_grad_out_feat[t * in_nchannel + cid]);
  }
}
...
void DepthwiseConvolutionForwardKernelGPU(
  ...
  // Iterate through each spatial kernel and get indices for in_map and out_map
  for (int k = 0; k < kernel_volume; k++) {
    int nmap_size = neighbor_offset[k];
    if (nmap_size == 0) continue;

    dim3 threads(SHARED_BLOCK_SIZE, SHARED_BLOCK_SIZE);
    dim3 grid((nmap_size + threads.x - 1) / threads.x, (in_nchannel + threads.y - 1) / threads.y);

    dwconv_forward<<<grid, threads, 0, stream>>>(
      d_in_feat, 
      d_out_feat,
...
void DepthwiseConvolutionBackwardKernelGPU(
  ...
  for (int k = 0; k < kernel_volume; k++) {
    ...
    dwconv_backward<<<grid, threads, 0, stream>>>(
      d_in_feat, 
      &d_kernel[k * in_nchannel],
      &neighbor_map[cur_offset], 
      nmap_size,
