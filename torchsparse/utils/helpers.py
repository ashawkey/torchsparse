...
'''
This essentially removed duplicated points in the same voxel ? [np.unique on hashed coords]
'''
    ...
    if isinstance(batch[0], dict):
        ...
        for key in batch[0].keys():
            if isinstance(batch[0][key], SparseTensor):
                ans_dict[key] = sparse_collate_tensors([sample[key] for sample in batch])
            elif isinstance(batch[0][key], np.ndarray):
                ans_dict[key] = torch.stack([torch.from_numpy(sample[key]) for sample in batch], axis=0)
