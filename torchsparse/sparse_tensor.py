...
class SparseTensor:
    def __init__(self, feats, coords, cur_tensor_stride=None):
        self.F = feats
        self.C = coords
        if cur_tensor_stride is not None:
            self.s = cur_tensor_stride
        else:
            self.s = np.array([1 for i in range(coords.shape[1])])
        self.coord_maps = {}
        self.kernel_maps = {}

    def check(self):
        if str(self.s) not in self.coord_maps:
            self.coord_maps[str(self.s)] = self.C
