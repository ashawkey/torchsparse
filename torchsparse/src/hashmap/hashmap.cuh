...
do_1st_hash(const VTYPE val, const int num_buckets) {
    /*
    if(num_buckets % 2) return val % num_buckets;
    return val % (num_buckets-1);   // Simply using modulo as 1st-level hashing.
    */
    
    //return val % (num_buckets-1); 
    return val % num_buckets;
}

inline __device__ int
do_2nd_hash(const VTYPE val, const FuncConfig * const hash_func_configs, const int func_idx,
            const int size) {
...
inline __device__ VTYPE
fetch_val(const VTYPE data, const int pos_width) {
    return data >> pos_width;
}

inline __device__ int
fetch_func(const VTYPE data, const int pos_width) {
    return data & ((0x1 << pos_width) - 1);
}

inline __device__ VTYPE
make_data(const VTYPE val, const int func, const int pos_width) {
    return (val << pos_width) ^ func;   // VTYPE CANNOT handle signed values currently!
}
...
private:
    ...
    inline VTYPE fetch_val(const VTYPE data) {
...
public:
    ...
    int insert_vals(const VTYPE * const keys, const VTYPE * const vals, VTYPE * d_key_buf, VTYPE * d_val_buf, VTYPE * d_key, VTYPE * d_val, const int n);
    // delete is not supported.
    void lookup_vals(const VTYPE * const keys, VTYPE * const results, VTYPE *d_key, VTYPE *d_val, const int n);
};
